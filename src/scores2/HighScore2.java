...
public class HighScore2 {
	...
	public BestPlayer[] tenBestScores(String[] readScores){//Les donnï¿½es en entrï¿½e doivent ï¿½tre de la forme"pseudo: score" sous peine de lever une Exception
		...
		int i;
		BestPlayer[] tabPlayer = new BestPlayer[readScores.length];
		String[] split;
		...
		for(i=0;i<readScores.length;i++){
			...
			tabPlayer[i] = new BestPlayer(split[0],Integer.parseInt(split[1]));
		}

		//Boucle pour trier le tableau. (tri à bulles)
		i=0;
		while(i<tabPlayer.length-1){
			if(tabPlayer[i].compareTo(tabPlayer[i+1]) == 1){ //le player à l'indice i est plus petit que celui à l'indice i+1
